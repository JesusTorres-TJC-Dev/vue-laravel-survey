<template>
    ...
    <form class="mt-8 space-y-6" @submit="login">
      <Alert v-if="errorMsg">
        {{errorMsg}}
        <span @click="errorMsg = ''" class="w-8 h-8 flex items-center justify-center transition-colors cursor-pointer hover:bg-[rgba(0,0,0,0.2)]">
    ...
    store.dispatch('login', user)
        .then((resp) => {
          if(resp.response && resp.response.status == 422){
            ///console.log(resp)
            loading.value = false
            errorMsg.value = resp.response.data.message
          }else{
            loading.value = false
            router.push({
              name: 'Dashboard'
